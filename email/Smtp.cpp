...
int CSMTPMessage::AddRecipient(CSMTPAddress& recipient, RECIPIENT_TYPE RecipientType)
{
  ...
  {
    case TO:  
		m_ToRecipients.push_back(recipient);  
		nIndex = m_ToRecipients.size() - 1;
		break;
    case CC:  
		m_CCRecipients.push_back(recipient);  
		nIndex = m_CCRecipients.size() - 1;
		break;
    case BCC: 
		m_BCCRecipients.push_back(recipient); 
		nIndex = m_BCCRecipients.size() - 1;
		break;
    default: ASSERT(FALSE);                            break;
  }
...
{
	return (CSMTPAttachment*) &m_Attachments.at(nIndex);
}
...
CString CSMTPMessage::GetHeader() const
{
  ...
  time_t tim; time(&tim);
  struct tm *today = localtime(&tim);
  char szDateString[255] = {0};

  strftime(szDateString, 255, _T("%a, %d %b %Y %H:%M:%S "), today);
  CString sDate(szDateString);
  sDate += sTZBias;
...
BOOL CSMTPConnection::SendMessage(CSMTPMessage& Message)
{
  ...
  for (i=0; i<Message.m_Attachments.size(); i++)
  {
  ...
  //retrieve the reponse using until we
    ...
    //If an error occurred receiving the data
      ...
      if (nBufRemaining-nData == 0) //No space left in the current buffer
      {
        ...
        LPTSTR pszNewBuf = new char[nBufSize];
